	...
	function flattenObject(key, obj = {}, flattenedObj = {}, prefixString = "") {
		if (typeof obj[key] != "object") {
			...
			Object.keys(obj[key]).forEach((eachKey) => {
				flattenedObj = flattenObject(eachKey, obj[key], flattenedObj, prefixString + key + "_");
			});
		} else {
			flattenedObj[prefixString + key] = obj[key];
		}
...
// Tags: regex, replace multiple, replaceAll, string, substring, sub string,
{
	function populateSubCategoryOptions() {
		...
		subCategoryList.forEach((optionData) => {
			let dataMap = { ":GUID": optionData["guid"], ":PARENT_GUID": optionData["parent_guid"], ":NAME": optionData["name"] };
			let tempOption = subCategoryOptionTemplate.replace(/:GUID|:NAME|:PARENT_GUID/g, (eachMatch) => dataMap[eachMatch]);
...
// Tags: get day name from date string, weekday
{
	function getDayName(dateInDDMMYYYY) {
		return new Intl.DateTimeFormat("en-Us", { weekday: "long" }).format(new Date(dateInDDMMYYYY));
	}
...
// To reverse the words in a sentence:
{
	function reverseWordsInSentence(sentence) {
		...
		split[split.length - 1] = split[split.length - 1].match(/[a-zA-Z0-9]/gi).length > 1 ? split[split.length - 1].toLowerCase() : split[split.length - 1];
		split = split.join(" ");
